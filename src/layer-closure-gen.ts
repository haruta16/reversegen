...
export function runLayerClosureGen(input: LayerClosureInput): LayerClosureOutput {
  ...
  const { matrix, retainedOldDebtTilesByLayer } = buildMatrixByCloseRates(
    colorTotalTiles, tilesPerDepth, closeRates, p,
  );
  ...
  // ── 7. 难度指标 ──
  const metrics = computeMetrics(assignments, freeTiles, depthLayers, depthMap, tileMap, tileDepSets, dock, colorCount, actualCloseRates, p, retainedOldDebtTilesByLayer);
...
function buildMatrixByCloseRates(
  ...
  for (let d = 0; d < D; d++) {
    ...
    if (target > currentlyCompleted) {
      ...
      const nextDebtTiles = Math.max(0, totalTilesAfter - target * 3);
...
export function computeMetrics(
  ...
  const { colorUsageRates, averageColorActivationLayer, debtTileCountsByLayer,
    ...
    computeLayerProgressMetrics(assignments, depthLayers);

  // 逐层保留旧债务 tile 数必须采用落色后的事实统计。
  // buildMatrix 阶段的记录只用于生成过程调试；兜底填充/容量约束可能让计划值与事实值略有偏差。
