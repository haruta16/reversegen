...
export async function runBatchGeneration(
  ...
  for (let i = 0; i < config.terrainPaths.length; i++) {
    ...
    try { terrain = loadTerrainFromFile(path); } catch (err) {
      tp.phase = 'done';
      const ep: GenerationParams = { closeRates: [], colorCount: 0, spreadParam: 0, debtPersistenceWeight: 0 };
      tp.rows.push(mkEmptyRow(i, path, ep, 0, false, false, err instanceof Error ? err.message : String(err)));
    ...
    tp.phase = 'maxgrade';
    if (onProgress) onProgress(progress);
    const seed = (Date.now() + i * 10000) & 0x7fffffff;
    const { maxGrade, row: probe } = determineMaxGrade(terrain, unified, i, path, config.simRuns, seed);
