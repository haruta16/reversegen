...
2. **`List<T>.Sort` 不稳定并列**：**分析器同深度 top-9 已修复为全序（`(Dock,深度,ID)`，
   两侧同步改）**；剩余并列（分配器随机键碰撞等）Unity 自身不确定，记录不修。
...
5. **Unity 端"同输入首轮泡泡不一致"的经典案例**：根因 = 分析器深度排序不稳定 +
   字典迭代序；修复方式 = 给排序补唯一键破平（ID）成全序，reversegen 镜像。
...
- ❌ 在 Unity 逻辑里使用 `ElementValue`（真实花色，表现层）做排序键/分组键/种子折叠——
  一律用 `LogicalElementValue`（归一化逻辑键）；花色在牌之间移动（洗牌重分配）时，
  逻辑键必须随花色包一起搬移；快照/克隆必须持久化逻辑键，否则恢复后键塌缩为真实花色。
